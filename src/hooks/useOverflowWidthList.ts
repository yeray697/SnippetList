...
const getTextWidth = (text: string, ref: React.RefObject<any>): number => {
  if (!ref.current?.children[0] && !context) return 0;
...
const useOverflowWidthList = <T extends Item>(
  ...
  useEffect(() => {
    if (!ref?.current || rect.clientWidth === 0) return;
    const additionalWidth =
      (theme.chip.borderWidth + theme.chip.textPaddingSides) * 2 +
      theme.chip.margin;
    let newList: T[] = [];
    let width: number = 0;

    const initialWidth = rect.clientWidth;
